...
frontend/
  ...
  pages/
    ...
    16_pipeline_dashboard.py ← Cross-project financial dashboard; budget vs billed vs invoiced
    17_receivables.py       ← Manage inter-office income splits and receivable payments
    18_recurring_fees.py    ← Cross-project recurring fee overview; skip, mark invoiced
...
| billing_arrangement | TEXT | `local` (hold contract, issue invoices) / `receivable` (another entity bills client; we track inter-office income) / `internal` (overhead, excluded from revenue reports) |
...
One row per recurring fee, linked to a project code.
...
| index_rate | REAL | Annual rate as decimal (e.g. 0.03 = 3%); used only when fee_type=indexed |
...
| billing_type | TEXT | `we_bill` (we invoice the client) / `third_party_bills` (another entity invoices; we receive a split) |
| split_party | TEXT | Name of the other Milliman entity (third_party_bills only) |
| split_amount | REAL | Our fixed share per period (third_party_bills) |
| auto_invoice | INTEGER | Reserved; always 0 (manual invoicing only) |
| status | TEXT | `Active` / `Cancelled` |
| cancelled_at | TEXT | |
| notes | TEXT | |

Indexed amounts: `fee_amount × (1 + index_rate) ^ (occurrence_year − start_year)`, stepped once per calendar year. All occurrences within a year share the same indexed amount regardless of frequency.

### `recurring_fee_occurrences`

One row per billing period per fee. Auto-generated by `_ensure_occurrences()` on a rolling 3-year horizon.
...
| amount | REAL | Indexed amount for this period (may be overridden per-occurrence) |
| split_amount | REAL | Our split for third_party_bills fees |
| invoice_id | INTEGER FK→invoices NULLABLE | Set when the occurrence is linked to an invoice |
| status | TEXT | `pending` / `invoiced` / `skipped` |
| notes | TEXT | |

Unique on `(recurring_fee_id, due_date)`. `INSERT OR IGNORE` prevents duplicates during generation.

Occurrence lifecycle: `pending` → `invoiced` (via `invoice_occurrence()`) or `skipped` (via `skip_occurrence()`).

### `receivables`

Expected inter-office income, one row per agreement.
...
| project_id | INTEGER FK→projects | Cascade delete |
| project_code_id | INTEGER FK→project_codes NULLABLE | |
| occurrence_id | INTEGER FK→recurring_fee_occurrences NULLABLE | Optional link to a recurring fee occurrence |
| description | TEXT | |
| expected_amount | REAL | Total amount expected |
| due_date | TEXT | ISO YYYY-MM-DD |
| receivable_type | TEXT | `capped` (pre-agreed amount) / `time_based` (tracks actual time charges) |
| cap_amount | REAL | |
| notes | TEXT | |

Outstanding balance = `expected_amount − SUM(receivable_payments.amount)` (computed at query time, not stored).

### `receivable_payments`

Individual receipts against a receivable.
...
| Function | Description |
|----------|-------------|
| `get_recurring_fees(project_code_id)` | Returns all fees (any status) for a project code as `list[dict]`. |
| `add_recurring_fee(project_code_id, ...)` | Inserts a new fee row; calls `_ensure_occurrences()` to generate the initial set. |
| `update_recurring_fee(fee_id, ...)` | Updates fee fields; deletes **all** pending occurrences then regenerates from scratch. |
| `cancel_recurring_fee(fee_id)` | Sets `status='Cancelled'` and `cancelled_at=now`. |
| `get_occurrences(fee_id)` | Returns all occurrences for a fee ordered by due_date ascending. |
| `skip_occurrence(occurrence_id, note)` | Sets `status='skipped'`. |
| `invoice_occurrence(occurrence_id, invoice_id)` | Sets `status='invoiced'` and records the linked invoice ID. |
| `update_occurrence_amount(occurrence_id, amount, split_amount, notes)` | Overrides the amount on a single pending occurrence. Guard: `WHERE status='pending'`. |
| `get_all_recurring_fees(status)` | Cross-project summary with client/project/code context; used by Page 18. Calls `_ensure_occurrences()` for each fee on load. |
| `_ensure_occurrences(conn, fee_id, horizon_years=3)` | Internal: generates occurrences from coverage_start forward to `today + horizon_years` (capped at expected_end). Uses `INSERT OR IGNORE` to skip duplicates. Indexed amounts are `base × (1 + rate) ^ (year − start_year)`. |
...
| Function | Description |
|----------|-------------|
| `get_receivables(project_id)` | Returns all receivables for a project. Each row includes `outstanding` (computed = expected_amount − SUM payments). |
| `add_receivable(project_id, ...)` | Inserts a new receivable. |
| `update_receivable(receivable_id, ...)` | Updates a receivable record in-place. |
| `delete_receivable(receivable_id)` | Deletes the receivable and all its payment records (via cascade). |
| `add_receivable_payment(receivable_id, amount, date, notes)` | Records an individual receipt. |
| `get_receivable_payments(receivable_id)` | Returns all payments for a receivable ordered by date. |
